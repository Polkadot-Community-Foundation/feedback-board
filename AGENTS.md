...
Feedback Board is a decentralized sticky-note board on Polkadot — notes are stored on Bulletin and indexed by a shared Asset Hub contract, so every remix sees the same board. (React + Vite + TypeScript, embedded in a Polkadot host — Mobile, Desktop, or Web; signing is approved on Polkadot Mobile). Canonical agent guidance is in [CLAUDE.md](CLAUDE.md) — read that first.
